...
app.post('/uploadDentalFile', async (req, res) => {
    try {
        const { patientID, filePath, fileName, fileType, uploaderID } = req.body;

        if (!patientID || !filePath || !fileName || !fileType || !uploaderID) {
        ...
        // Call the chaincode function to store file metadata
        await contract.submitTransaction(
            'addDentalFile',
            patientID,
            cid,
        ...
        res.status(200).json({ message: 'File uploaded and metadata stored on Fabric', cid });
        await gateway.disconnect();
    } catch (error) {
        console.error(`Error uploading dental file: ${error}`);
